...
checkSingleBracketOperators params (TC_Binary id SingleBracket "=~" lhs rhs)
    | shellType params `elem` [Bash, Ksh, IrixKsh, IrixDtksh] =
...
isIrixIntegerVariable params token =
    shellType params `elem` [IrixSh, IrixKsh, IrixDtksh] && fromMaybe False (do
        name <- getLiteralString token
        guard $ isVariableName name
        cfga <- cfgAnalysis params
...
checkComparisonAgainstGlob params (TC_Binary _ SingleBracket op _ word)
  ...
  where
    msg = if (shellType params) `elem` [Bash, Ksh, IrixKsh, IrixDtksh]  -- Busybox does not support glob matching
...
prop_checkSpacefulnessCfg83 = verifyNot checkSpacefulnessCfg "case \"$1\" in foo|bar) echo $1;; esac"
prop_checkSpacefulnessCfg84 = verifyNot checkSpacefulnessCfg "case \"$value\" in foo|bar) echo $value;; esac"
prop_checkSpacefulnessCfg85 = verify checkSpacefulnessCfg "case \"$1\" in foo|'has space') echo $1;; esac"
prop_checkSpacefulnessCfg86 = verify checkSpacefulnessCfg "case \"$1\" in foo|'*') echo $1;; esac"
prop_checkSpacefulnessCfg87 = verify checkSpacefulnessCfg "case \"$1\" in '*') echo $1;; esac"
prop_checkSpacefulnessCfg88 = verify checkSpacefulnessCfg "case \"$1\" in '') echo $1;; esac"
prop_checkSpacefulnessCfg89 = verify checkSpacefulnessCfg "case \"$value\" in foo) value='has space'; echo $value;; esac"
prop_checkSpacefulnessCfg90 = verify checkSpacefulnessCfg "case \"$1\" in foo) set -- 'has space'; echo $1;; esac"
prop_checkSpacefulnessCfg91 = verify checkSpacefulnessCfg "case \"${10}\" in foo) shift; echo ${10};; esac"
prop_checkSpacefulnessCfg92 = verify checkSpacefulnessCfg "case \"$value\" in foo) :;; esac; echo $value"
prop_checkSpacefulnessCfg93 = verify checkSpacefulnessCfg "case \"$value\" in 'has space') ;& bar) echo $value;; esac"
prop_checkSpacefulnessCfg94 = verify checkSpacefulnessCfg "IFS=o; case \"$1\" in foo|bar) echo $1;; esac"
prop_checkSpacefulnessCfg95 = verifyNot checkSpacefulnessCfg "IFS=z; case \"$1\" in foo|bar) echo $1;; esac"
prop_checkSpacefulnessCfg96 = verifyNot checkSpacefulnessCfg "f() { case \"$1\" in foo) echo $1;; esac; }; f foo"
prop_checkSpacefulnessCfg97 = verify checkSpacefulnessCfg "f() { case \"$1\" in foo) :;; esac; }; f foo; echo $1"
...
prop_checkUnused21a = verifyNotTree checkUnusedAssignments "a=1; trap '(( a ))' INT"
prop_checkUnused21b = verifyNotTree checkUnusedAssignments "a=1; trap 'echo $((a + 1))' INT"
prop_checkUnused21c = verifyNotTree checkUnusedAssignments "a=1; trap 'case x in x) echo $a;; esac' INT"
prop_checkUnused21d = verifyTree checkUnusedAssignments "a=1; trap 'echo a' INT"
prop_checkUnused21e = verifyTree checkUnusedAssignments "a=1; trap 'a=2' INT"
prop_checkUnused21f = verifyTree checkUnusedAssignments "action='echo a'; a=1; trap \"$action\" INT"
prop_checkUnused21g = verifyTree checkUnusedAssignments "a=1; trap '(( a = 2 ))' INT"
prop_checkUnused21h = verifyTree checkUnusedAssignments "a=1; trap 'a=2; echo $a' INT"
prop_checkUnused21i = verifyNotTree checkUnusedAssignments "a=1; trap 'echo $a; a=2' INT"
prop_checkUnused21j = verifyNotTree checkUnusedAssignments "a=1; trap 'if ok; then a=2; fi; echo $a' INT"
prop_checkUnused21k = verifyNotTree checkUnusedAssignments "a=1; trap '(a=2); echo $a' INT"
prop_checkUnused21l = verifyNotTree checkUnusedAssignments "tmp=x; trap \"echo '$tmp'; status=\\$?; echo \\$status\" EXIT"
...
prop_checkUnassignedReferencesTrapRead = verifyTree checkUnassignedReferences "trap 'echo $status' EXIT"
prop_checkUnassignedReferencesTrapLocal = verifyNotTree checkUnassignedReferences "trap 'status=$?; echo $status' EXIT"
prop_checkUnassignedReferencesTrapReadFirst = verifyTree checkUnassignedReferences "trap 'echo $status; status=$?' EXIT"
prop_checkUnassignedReferencesTrapConditional = verifyTree checkUnassignedReferences "trap 'if ok; then status=$?; fi; echo $status' EXIT"
prop_checkUnassignedReferencesTrapSubshell = verifyTree checkUnassignedReferences "trap '(status=$?); echo $status' EXIT"
prop_checkUnassignedReferencesTrapMixed = verifyNotTree checkUnassignedReferences "tmp=x; trap \"echo '$tmp'; status=\\$?; echo \\$status\" EXIT"
...
checkFunctionDeclarations params
    ...
    case shellType params of
        ...
        IrixDtksh ->
            when (hasKeyword && hasParens) $
                err id 2111 "IRIX dtksh does not allow 'function' keyword and '()' at the same time."
        IrixSh ->
            when (hasKeyword && hasParens) $
...
checkTestArgumentSplitting params t =
  ...
  where
    ...
    checkNumericalGlob SingleBracket token =
        ...
        when (shellType params `notElem` [Ksh, IrixSh, IrixKsh, IrixDtksh] && isGlob token) $
  ...
  where
    ...
    checkPart part = case part of
        ...
        T_DollarBraced id _ str |
            ...
            -> warn id 2206 $
                if shellType params `elem` [Ksh, IrixSh, IrixKsh, IrixDtksh]
  ...
  where
    ...
    assign0Msg id bashfix =
        case shellType params of
            ...
            IrixDtksh -> err id 2278 "$0 can't be assigned in IRIX dtksh (but it does reflect the current function)."
...
checkRequireDoubleBracket params =
    if (shellType params) `elem` [Bash, Ksh, IrixKsh, IrixDtksh, BusyboxSh]
...
prop_checkSetESuppressedIrixDtkshCommandSubstitution = verifyNotTree checkSetESuppressed
    "# shellcheck shell=irix-dtksh\nset -e; f(){ false; print survived; }; value=$(f)"
...
checkStatusAfterCommandSubstitution params (T_SimpleCommand _ _ words@(_:_)) =
    when (shellType params `elem` [Bash, Ksh, IrixKsh, IrixDtksh]) $ checkEvents False $ concatMap events words
  where
